...
public partial class UnitConfig
{
	...
	public static Dictionary<int, UnitConfig> ReturnDictionary(string csv)
	{
		...
		for (int i = 3; i < reader.Row; i++)
		{
			...
			item.Type = paras1;
			item. = reader.GetData(2, i);
			item.Name = reader.GetData(3, i);
			item.Desc = reader.GetData(4, i);
			int.TryParse(reader.GetData(5, i), out int paras2);
			item.Position = paras2;
			int.TryParse(reader.GetData(6, i), out int paras3);
			item.Height = paras3;
			int.TryParse(reader.GetData(7, i), out int paras4);
			item.Weight = paras4;
			try
			{
